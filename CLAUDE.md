...
- **State**: Zustand stores in `stores/`:
  ...
  - `skills-data.ts` — skills data, update checks, update cache, and install/update actions. Reads context via `useContextStore.getState()` (cross-store access pattern).
  - `skill-dialog.ts` — delete, manage agents, copy to project, and repair source dialog state/actions.
  - `skills.ts` — backward-compatible barrel that re-exports split skill stores.
  - `settings.ts` — theme/locale (persisted to localStorage via `partialize`), scope-aware default target agents (persisted to backend `~/.agents/.skill-lock.json` as `defaultTargetAgents`, with `lastSelectedAgents` kept as CLI-compatible fallback).
...
- Key types: `InstalledSkill`, `AgentInfo`, `AgentType` (54 agent variants), `AgentScopeTarget`, `DefaultTargetAgents`, `AppError`, `InstallParams`, `Scope`
...
- Multi-step wizard: `components/skills/add-skill/` (ScopeStep → SourceStep → SkillsStep → OptionsStep → ConfirmStep → InstallingStep → CompleteStep/ErrorStep)
...
```
ScopeStep → SourceStep → SkillsStep → OptionsStep → ConfirmStep → InstallingStep → CompleteStep/ErrorStep
```

- **ScopeStep**: 用户选择安装范围。Global 使用 `~/.agents/skills`，Project 使用目标项目的 `.agents/skills`
...
- **OptionsStep**: 选择额外安装到哪些 agents。auto-detect 系统中已安装的 agents（54 种），按当前 scope 读取默认安装目标（持久化在 `~/.agents/.skill-lock.json` 的 `defaultTargetAgents`）
- **ConfirmStep**: 展示最终确认摘要，区分自动可用的 agents 和额外安装的 agents。调用 `checkOverwrites()` 检测是否有同名 skill 已存在
- **InstallingStep**: 调用 `installSkills()` 执行安装，支持 overwrite 已存在的 skill
- **CompleteStep/ErrorStep**: 展示安装结果

Agent 目标规则：

- CLI 的 `universal` 是静态分类：`skillsDir === ".agents/skills"`。GUI 不把 CLI 的 `universal` agent key 当作真实目标。
- GUI 的自动可用 Agent 是 scope-aware：某个 Agent 在当前 scope 下的目标路径等于共享目录时才算自动可用。
- OptionsStep 只保存额外 Agent；安装时后端会按当前 scope 自动补齐自动可用 Agent。
- 同一个 Agent 在不同 scope 下可能分类不同，例如项目中自动可用、全局中需要额外安装。
...
- **自动应用 Agent 分组**: 分组逻辑 MUST 与 install wizard 的 ConfirmStep 一致。自动应用的 agents 读取当前 scope 的共享目录，额外 Agent 才需要用户选择或部分移除
...
- **新增/修改 store action** → 检查跨 store 引用：`context.ts` ↔ `skills-data.ts` / `skill-dialog.ts`（skill stores 读 context 的 getState()）、`settings.ts`（独立持久化）
