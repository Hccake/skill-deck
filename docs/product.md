...
主窗口提供 `Skills`、`Skill 库`、`Discover` 和 `Settings` 四个一级页面，并集中展示当前安装流程、写操作状态和恢复入口。Windows 启用 WSL 支持并发现发行版后，主窗口还会显示 Environment 切换入口；`Skills`、`Skill 库`、`Discover` 以及 `Settings` 中的 Agents 和 Projects 使用用户当前选择的 Windows 或 WSL 位置，通用设置、Git 和关于页不依赖该选择。
...
`Skill 库` 是独立于全局 Skill 和项目 Skill 的一级页面。用户可以在当前 Environment 中创建多个库，从来源添加或移除 Skill，检查当前库中全部成员的更新，并更新其中有新版本的成员。

用户向当前库添加 Skill 时，主窗口打开应用级模态流程并固定当前 Environment、库和已有成员。流程依次完成来源读取、Skill 选择和添加确认；模态打开期间，用户不能操作背景导航、库列表、Environment 切换或其他页面内容。来源读取和确认准备期间保持当前流程可见，内容写入期间不能关闭模态或退出应用。

来源步骤与安装向导使用相同的在线搜索、手动输入和 `skills add` 命令解析能力。一个来源包含多个 Skill 时，选择列表只把可加入当前库的成员计入可选数量；当前库已有的同名 Skill 持续显示为不可选项。命令中的 Skill、wildcard 和 Pack 意图用于初始选择；Agent 参数不改变库成员，界面会说明用户可以在添加完成后从 `Skills` 页应用该库。

确认页列出本次写入的 Skill 和目标位置。长路径在列表中截断，并通过可聚焦提示展示完整内容；下载地址跨主机跳转时，用户需要显式确认最终主机。执行结果按 Skill 展示，部分失败时只重试失败项，并保留此前已经成功的结果。

页面由库列表和当前库的工作区组成。库列表每一行显示库名和该库的应用状态：已确认生效时显示应用位置数量，只有未完成的应用变更引用它时显示应用调整未完成，两者皆无时显示未应用。行内提供重命名和删除入口。

当前库的名称、应用状态、成员数量、更新概况和主要操作固定显示在工作区顶部，列表滚动时保持可见。成员数量始终表示库的全部成员，关键词筛选的命中数量显示在搜索区域。应用状态在空间充足时展示已经生效的位置名称；空间不足或位置较多时收敛为位置数量，并通过可聚焦的展开入口查看具体位置和未完成的调整。展开列表使用图标区分全局和项目位置；项目名称采用统一项目展示规则，完整路径作为次要定位信息。库尚未被应用时，页面明确显示未应用并说明可以在 `Skills` 页的全局或项目区域应用它。最窄窗口使用稳定的双行顶部布局，操作和状态不会自由换行或相互遮挡。

整库检查分别显示有更新、没有更新、无法检查和上游已删除。没有需要处理的类别时显示“全部已是最新”；需要处理的类别较多时合并其余项。检查失败时，页面保留失败前最近一次成功得出的成员状态，并提示本次检查未完成；用户确认更新后才会取得所选成员的完整内容。

成员卡片显示名称、简介、来源和 Skill Deck 最近一次提交该成员内容的时间，并常驻更新与移除两个操作。更新在尚未检查、已是最新、无法检查和上游已删除时保持不可用，并说明当前原因。用户可以点击卡片或成员名称查看该成员的 `SKILL.md`。

库正在被使用时，添加成员、移除成员和删除库保持不可用，界面在对应入口说明原因。已确认生效和存在未完成调整都会形成这项限制。用户从相关位置解除应用后，可以调整库成员或删除库。

删除入口固定使用用户点击的库作为目标，确认期间不会跟随当前选中库变化。删除未选中的库时继续显示当前库；删除选中库时优先选择原位置的下一项，没有下一项时选择上一项。删除最后一个库后页面进入空状态。删除失败时确认界面保留目标并显示原因；删除成功后的详情读取失败只提供详情重试，不再次执行删除。页面 URL 始终跟随已经提交的库选择。

Skills 页面在项目区域和全局区域的标题中分别提供“管理 Skill 库”入口。用户可以在当前区域选择多个非空库、调整优先顺序，并为只读取 Agent 专用 Skill 目录且支持链接的 Agent 统一选择应用位置。当前应用库位于区域标题下方、直接安装 Skill 列表上方；每个库通过统一图标、库名和成员数量表达身份，排列顺序表示优先级，不再使用额外标题或横向分隔线。库摘要只展示当前应用状态，不提供页面跳转；查看和维护库内容通过一级“Skill 库”页面完成。界面直接展示当前区域应用的全部库；普通列表空间不足时允许库条目换行，紧凑列表按库逐行展示。当前区域没有应用库时不显示库摘要，未完成的库应用变更仍持续显示。关键词搜索和 Agent 筛选只改变直接安装 Skill 列表，库摘要持续显示。

管理弹窗在桌面宽度下并列展示 Skill 库和关联 Agent：左侧区分当前应用库与未应用库，当前应用库的顺序表示优先级；右侧复用统一的 Agent 名称、分组和检测状态展示。窄窗口将两部分改为上下排列，两部分分别滚动，标题和操作区保持固定。库条目使用与外部摘要一致的图标、名称和成员数量；空库继续显示在未应用库中，但不能选择。用户修改配置后直接保存，应用在后台依次完成预览校验和执行，不展示独立确认页；通用 Skill 目录保持直接版本时，Agent 专用目录发生的库版本新增、移除或切换仍然属于本次变更结果。保存成功后刷新 Skills 页面并关闭弹窗。目标无法安全处理时，弹窗保留当前选择，说明受影响的 Skill 和 Agent，并允许取消对应 Agent 关联；状态过期或临时失败时提供与原因相符的重新加载或重试操作。取消或关闭弹窗会直接放弃尚未保存的修改，保存期间不能关闭弹窗；加载和未完成操作分别提供对应反馈与后续操作。

Scope 表示当前 Environment 中的全局或项目管理范围，包含通用 Skill 目录和 Agent 专用 Skill 目录。同名 Skill 在每个实际目录中分别进行版本选举：通用 Skill 目录优先使用直接安装版本；Agent 专用 Skill 目录只在直接 Skill 已关联该目录时优先使用直接版本。没有适用的直接版本时，系统按照当前应用库的顺序使用第一个同名库版本；没有可用版本时，该目录不再保留这个 Skill。

安装或移除直接 Skill、增删直接 Skill 的关联 Agent、应用或取消应用库、调整库顺序以及增删库的关联 Agent，都会让受影响的目录重新进行版本选举。库应用关系在直接版本优先期间继续保存，因此移除直接版本或取消直接 Agent 关联后，相应目录会恢复当前优先级最高的库版本。多个 Agent 共用同一个实际目录时，系统只更新一次目录；目录已经符合选举结果时不会重复写入，但仍会在提交应用状态前复核目录是否发生变化。

有效的现有 Skill 目录、符号链接或 junction 会作为该目录中的直接版本保留，以兼容旧版 Skill Deck 和其他兼容工具已经创建的 Agent 安装项。失效链接会在存在可用版本时替换为新的直接安装项或库链接；文件等不能作为 Skill 目录处理的条目会阻止对应目录更新，并显示受影响的 Skill 和 Agent。库应用支持能够通过链接读取标准内容的 Agent；需要复制或转换内容的 Agent 继续通过直接安装流程取得 Skill。
...
4. 用户查看安装后可以直接使用 Skill 的 Agent，并按需选择要为哪些 Agent 创建链接或副本。Eve 项目还会展示根 Agent 和已经发现的子 Agent。向导根据入口信息、检测结果和当前 Environment 的最近选择生成初始状态，用户可以在本次安装中调整选择；完整规则见[Agent 模型](./agent-model.md#安装初始选择与最近选择)。
...
- Git 设置允许验证并保存 GitHub Token。已经保存的 Token 不回显；私有 GitHub 来源还可以使用本机 Git 已有的凭据检查更新。凭据不可用时，界面说明可使用的其他凭据来源。
...
- 删除用户添加的信息时，现有 Skill 文件继续保留。确认页说明相关目录、Skill 数量和后续管理影响。完整领域规则见[Agent 模型](./agent-model.md)。
...
Skill 库应用使用当前 Environment 原生管理且能够安全创建链接的本地目录。Native UNC、WSL 的 Windows 挂载目录和跨发行版位置在库应用中显示为不可用目标。
