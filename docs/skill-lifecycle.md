...
普通 HTTP 或 HTTPS 地址会先按 Well-known 来源读取；该方式无法取得内容时，应用继续尝试受限的直接下载。托管平台明确表示制品的地址直接进入下载流程。直接下载支持原始 `SKILL.md`、ZIP、tar 和 tar.gz，下载与解包均有体积和条目数量上限；归档中的绝对路径、路径穿越、链接、重复路径以及跨平台名称冲突会被拒绝。重定向后的最终主机与原地址不同时，用户需要在安装确认页确认该主机。
