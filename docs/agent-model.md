...
Agent 安装选项是 Agent 专用 Skill 根目录的统一聚合结果。安装、复制、管理 Agent、移除和库应用复用同一份选项与已解析目录位置，再追加具体 Skill 目录名；这些流程不会重新解释 Agent 的私有路径或 Eve 目标。Agent ID 只说明目录的读取者和用户可见归属，不参与同一物理目录中的版本胜出判断。
...
管理 Agent 时，应用还会检查每个安装位置中已有的链接、副本、失效链接和无法处理的内容。这些现有状态不会影响选项属于“选择后可使用”还是“同时写入 Agent 自己的 Skill 目录（可选）”。为了兼容旧版 Skill Deck 和其他兼容工具创建的 Agent 安装项，有效目录、符号链接或 junction 按直接版本处理；文件等不能作为 Skill 目录处理的条目保持不变，并阻止对应目录更新。当前版本没有为历史 Agent 安装项增加独立的所有权记录。
...
Skill 库通过通用 Skill 目录自动提供给读取该目录的 Agent。只读取 Agent 专用 Skill 目录、使用标准内容并支持链接的 Agent 可以在当前 Scope 的库管理界面中统一选择；同一选择作用于该 Scope 中全部需要使用库版本的 Skill。多个 Agent 解析到同一物理目录时合并为一次写入，并保留全部 Agent 身份。

Skill 库应用支持能够通过链接读取标准内容的 Agent；需要复制或内容转换的 Agent 继续通过直接安装流程取得 Skill。Agent 专用目录只在直接 Skill 明确关联该物理目录时优先使用直接版本；否则使用关联到该目录且顺序最靠前的库版本。用户增删直接 Skill 的关联 Agent，或者增删库的关联 Agent 后，系统都会为相关目录重新进行版本选举。

管理直接 Skill 的 Agent 时，Checkbox 只表示直接 Skill 是否关联该安装位置。目录当前链接到任一已知库版本时，Checkbox 不选中，界面单独说明当前使用库版本；即使链接尚未切换到顺序最靠前的库版本，也不会将它误判为直接关联。用户取消直接关联后，相应目录恢复库版本或不再保留该 Skill。自定义 Agent 被库应用选择后，显示名称和检测设置仍可修改；用户从所有引用位置取消选择后，可以修改 Agent ID、全局或项目读取位置，或者删除该 Agent。随应用提供的 Agent 通过 `OfferMigration` 旧路径声明支持明确的重新应用，重新应用当前 Scope 时清理能够确认所有权的旧链接。
...
删除流程会列出选择该 Agent 的 Skill 库应用。用户从这些位置取消选择后，可以继续删除 Agent 信息。
...
11. Skill 库向通用目录和用户明确选择的可链接专用目录发布标准内容，需要复制或转换的目标继续使用直接安装流程。
12. 同名 Skill 在每个物理目录中分别进行版本选举；直接版本只在直接 Skill 实际关联的目录中优先，库版本按 Scope 保存的库顺序选择。
