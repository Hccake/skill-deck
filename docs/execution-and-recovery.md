...
安装、更新、复制、移除和调整 Agent 关联都会修改 Skill 目录或相关 lock 文件。Skill Deck 在写入前重新检查目录、lock、Agent 选择和当前运行状态，并把每个 Skill 或目标项目作为可以独立完成的写入任务。写入失败后，如果系统无法确认 Skill 目录与 lock 仍然一致，应用会保留恢复记录，供用户检查和清理。
...
安装以及在 Windows 与 WSL 之间复制时，会在生成预览前固定内容。更新先根据当前本地状态生成预览，用户确认后再取得新版本内容；取得内容后，后续检查和写入始终使用同一份快照。
...
同一发现会话中的同一个 Skill 共用正在进行的内容准备任务，后续请求等待同一结果。不同 Skill 可以分别准备；不同发现会话、Environment 或 WSL Worker 代次之间保持隔离。已有快照的复用继续校验来源证据，安装预览仍根据本次目标和选择重新生成。

调用方离开后，已经启动的内容准备和校验任务继续负责 I/O 与必要清理，其他调用方仍可等待共享结果。临时占用在实际 I/O 完成后释放，内容通过清单和容量检查后才发布。准备失败时清理未发布内容，再允许重试；清理失败时保留会话的收尾责任，并拒绝复用该会话。

安装准备遇到发现会话过期或内容失效时，需要重新获取来源和 Skill 列表，再核对用户选择。目标状态变化时可以保留有效内容，重新生成预览并要求确认。
...
安装、复制、移除和调整 Agent 关联在执行前要求预览凭据与当前状态完整一致。更新在取得新版本内容前也执行完整校验；内容取得后继续校验操作位置和运行时修订，并逐个 Skill 比较本地内容与来源记录。只有发生变化的 Skill 会被标记为结果过期，其余 Skill 仍可继续更新。
...
应用继续识别旧版本来源修复操作留下的恢复记录，允许用户查看和清理对应数据；当前版本不再创建新的来源修复操作。
