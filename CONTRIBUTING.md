...
感谢参与 Skill Deck 开发。本项目基于 Tauri，前端使用 React/TypeScript，后端使用 Rust。每项变更都需要同步维护用户行为、生成的 IPC 契约、窗口 ACL 和跨平台语义，并在涉及的平台上提供相应验证。
...
- 手动排查真实 WSL 行为时，需要在 Windows 开发机上准备 WSL2 和一个满足支持基线的发行版。仓库的常规自动验证使用跨平台协议测试和 Linux 脚本测试。
...
2. 修改共享函数、类型或模块前，先确认直接调用方、受影响流程和公共契约，再确定改动范围。
3. 功能和缺陷修复遵循测试先行（test-first），先让目标测试因缺少行为而失败，再实现最小改动。
4. 同一变更同步维护 Rust 公共契约、生成的 bindings、前端、ACL、国际化文案和长期文档。
5. 在有未提交改动的工作区中，保持任务范围清晰，只修改和验证本任务文件，并保留其他人的现有改动。
6. 提交前核对实际差异和受影响文件，再按改动风险运行相应验证。

设计说明、实施计划和阶段性评审保存在 Git 忽略的 `docs/plans/**` 或 `docs/superpowers/**` 中。源码、测试和对应的长期主文档共同记录最终行为，普通提交只包含这些长期内容。

## 前端

前端负责呈现和用户交互，后端负责文件系统、Agent 解析、授权和写入等业务规则。
...
- 页面、`Settings` 页面区域和高成本功能按路由或实际使用时机加载，应用启动阶段只获取首屏所需数据。
- 加载、空状态、错误、部分完成、过期和恢复状态使用稳定布局，并提供与当前状态对应的下一步。请求失败显示具体失败对象和处理入口。
- Windows/WSL、内置/自定义等技术差异在用户需要作出决策时展示，内部实现分层由代码和技术文档说明。

### 状态与异步请求

- Zustand 使用字段级选择器，避免订阅整个 store。
- Environment、Context 或请求键变化后，异步响应必须使用请求代次或请求标识，防止旧结果覆盖新状态。
- Promise 拒绝进入用户可见错误或明确的业务降级。连接、存在性和写入失败都需要对应处理结果。
- 跨组件共享的运行时事件、窗口重新获得焦点后的刷新、缓存和进行中的请求去重由对应的 store 或 hook 统一负责。
- 组件保存用户意图，后端的预览与执行结果才决定文件系统和变更事实。

### 国际化与文案

- 用户可见文案统一进入 `src/i18n/locales/en.json` 和 `zh-CN.json`，组件通过国际化键引用文本。
- 中文使用自然陈述，避免逐词翻译。Agent、Skill、Environment、Context、WSL 等产品或工程术语可以保留原文。
- 后端返回稳定的错误或警告代码及参数，前端根据代码和参数生成本地化反馈；内部英文摘要保留给开发信息和本机日志。
...
- 公共失败使用 `AppError` 或稳定的操作代码，前端行为由结构化代码和参数驱动。
- 读取、打开、删除和恢复命令接收类型化标识，后端根据标识重新解析并验证目标；前端路径只用于展示。
- 所有 Skill 变更复用预览与执行流程、计划器、协调器和恢复机制，目录与 lock 写入集中在执行层完成。
- 内容快照（`Payload`）表示完整的 Skill 目录，所有落盘方式都处理完整目录内容。
- WSL 操作使用随应用发布的脚本资源、类型化请求与响应、位置参数或结构化标准输入。用户输入作为数据传入脚本。
...
Main 与 Install Wizard 共享双方实际使用的安装与运行时命令；Settings、Recovery、Updater 等能力按窗口维持最小权限。每项授权都应有真实调用点，并同时进入应用命令清单、permission 和 capability。

## 文档

长期文档按规则所属主题划分，具体关系见[文档地图](./docs/README.md)。行为变化进入对应主文档，其他位置保留必要摘要和链接。
...
正文使用符合中文语境的陈述体，具体写作规则见[文档地图](./docs/README.md#写作规则)。复杂结构和时序可以使用 Mermaid，简单规则使用正文或表格。完整命令、类型、组件、Agent 和制品清单由源码与生成工具维护。
...
该命令静态检查随 Windows 应用发布的目标环境脚本。实际执行契约由 Linux 上运行的 Rust 测试覆盖；Windows 和 macOS CI 验证各自需要编译和运行的平台分支。真实 WSL 环境用于问题复现和补充验收。
...
原生集成测试使用真实的临时文件系统验证完整流程。WSL 的常规证据包括跨平台 Rust 解析器与协议测试、ShellCheck、Linux 上执行随应用发布的会话脚本，以及 Windows、macOS、Linux 三平台 Rust 构建矩阵。真实发行版验收用于特定问题复现和人工补充验证。
...
维护者确认 Draft Release 的版本、平台制品、签名和 `latest.json` 后，通过 GitHub UI 手动公开。发布门禁使用前述自动化协议、shell 验证和三平台构建矩阵；真实 WSL 发行版用于补充验收。

应用更新器依赖 Tauri 的签名校验。不得发布缺少签名的安装包，也不得手工拼接与当前标签不一致的 `latest.json`。

## 提交内容

- 提交包含当前任务需要的源码、测试、配置、长期文档和手写权限文件。
- 构建输出、临时证据、上游 CLI 的本地辅助文件、`docs/plans/**` 和 `docs/superpowers/**` 保留在本地。
- `src-tauri/permissions/autogenerated/**` 由 Tauri 根据 `app_commands.rs` 生成；提交手写的窗口命令集合和 `capability`。
- 每个提交围绕一个明确主题组织，无关格式化、生成文件噪声和待确认重构留给后续任务。
- Rust 契约变化时重新生成 bindings，并将生成结果与契约修改放在同一提交。
- 发布流程或 CI 变更继续覆盖 Rust、WSL 和更新器门禁；门禁调整需要在同一变更中说明新的验证责任。
